...
// ---------------------------------------------------------------------------
// Programme setup and corrections (previously untested)
// ---------------------------------------------------------------------------
import { programStart } from "./schedule";
import { adherence } from "./stats";

const MAP = { 0: "REST", 1: "UB1", 2: "LB1", 3: "REST", 4: "UB2", 5: "LB2", 6: "REST" };

describe("SETUP_PROGRAM", () => {
  it("stores the start date and defaults it to today", () => {
    const s = reducer(defaultState(), { type: "SETUP_PROGRAM", weekdayMap: MAP, week: 1, todayStr: "2026-08-21", backfill: false, now: 1 });
    expect(s.program).toMatchObject({ startDate: "2026-08-21", deloadEvery: 6 });
    expect(s.program.anchor).toEqual({ date: "2026-08-21", week: 1 });
  });
  it("accepts an explicit earlier start date", () => {
    const s = reducer(defaultState(), { type: "SETUP_PROGRAM", weekdayMap: MAP, week: 3, todayStr: "2026-08-21", startDate: "2026-08-03", backfill: false, now: 1 });
    expect(programStart(s.program)).toBe("2026-08-03");
  });
  it("starting today writes no back-filled days for earlier in the same week", () => {
    const s = reducer(defaultState(), { type: "SETUP_PROGRAM", weekdayMap: MAP, week: 1, todayStr: "2026-08-21", backfill: true, now: 1 });
    expect(Object.keys(s.days)).toEqual([]);          // the reported bug
    expect(adherence(s, "2026-08-21").scheduled).toBe(0);
  });
  it("back-filling from an earlier start writes assumed days from that date on", () => {
    const s = reducer(defaultState(), { type: "SETUP_PROGRAM", weekdayMap: MAP, week: 3, todayStr: "2026-08-21", startDate: "2026-08-10", backfill: true, now: 1 });
    const dates = Object.keys(s.days).sort();
    expect(dates[0]).toBe("2026-08-10");
    expect(dates).not.toContain("2026-08-03");        // before the start
    expect(Object.values(s.days).every(d => d.status === "assumed")).toBe(true);
  });
});

describe("FIX_WEEK", () => {
  const setup = () => reducer(defaultState(), { type: "SETUP_PROGRAM", weekdayMap: MAP, week: 1, todayStr: "2026-08-21", backfill: false, now: 1 });
  it("renumbers the weeks and leaves the start date alone", () => {
    const s = reducer(setup(), { type: "FIX_WEEK", todayStr: "2026-08-21", actualWeek: 5, backfill: false, now: 2 });
    expect(programStart(s.program)).toBe("2026-08-21");
    expect(Object.keys(s.days)).toEqual([]);
    expect(adherence(s, "2026-08-21").scheduled).toBe(0);   // no retroactive penalty
  });
  it("with back-fill it moves the start back to week 1 and fills those days", () => {
    const s = reducer(setup(), { type: "FIX_WEEK", todayStr: "2026-08-21", actualWeek: 3, backfill: true, now: 2 });
    expect(programStart(s.program)).toBe("2026-08-03");     // Monday of the new week 1
    expect(Object.keys(s.days).length).toBeGreaterThan(0);
  });
});

describe("SET_START_DATE", () => {
  it("moves the boundary without touching anything else", () => {
    const s = reducer(base(), { type: "SET_START_DATE", date: "2026-08-19" });
    expect(programStart(s.program)).toBe("2026-08-19");
    expect(s.program.weekdayMap).toEqual(program.weekdayMap);
  });
});
